...
async function generateAgentApplication(conversation, userInput) {
  const spinner = ora({
  ...
  try {
    ...
    if (!result || !result.success) {
      throw new Error("The agent failed to generate the application.");
    }

    console.log();

    console.log(`  ${accent("◇")} ${chalk.bold("Application Generated")}`);
    ...
    if (result.commands?.length > 0) {
      console.log(`  ${amber("▸")} ${chalk.bold("Setup Commands")}`);

      console.log();

      console.log(
        indentBlock(
          result.commands.map((command) => chalk.cyan(command)).join("\n"),
        ),
      );
    }
...
async function agentLoop(conversation) {
  ...
  while (true) {
    ...
    try {
      await generateAgentApplication(conversation, userInput);
...
export async function startAgentChat(conversationId = null, mode = "agent") {
  try {
    intro(
      boxen(
